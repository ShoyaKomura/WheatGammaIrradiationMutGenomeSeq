...
ghm <- ghm + geom_tile()
ghm <- ghm + theme_bw()
ghm <- ghm + scale_y_continuous(breaks = seq(0, l[[file_name]]/1000000, by=100), limits=c(0,l[[file_name]]/1000000), expand = c(0,0))+
  scale_x_continuous(expand=c(0,0), breaks=NULL)
ghm <- ghm + theme(panel.background = element_blank(),
                   panel.grid.minor = element_line(color = NA),
                   panel.grid.major = element_line(color = NA),
                   plot.background = element_blank(),
                   legend.title=element_blank(),
                   axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
ghm <- ghm + scale_fill_gradientn(colours=c("white","black"),limits=c(0,density_limit),values=rescale(c(0,1,5)))
ghm <- ghm + xlab(file_name) + ylab('position(Mbp)')
ggsave(save_figname, ghm, width = 2, height = 10, dpi=300)}
